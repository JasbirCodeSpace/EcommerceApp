...
class Index(View):
    ...
    def customer_get(self, request):
        ...
        return render(request, 'dashboard/customer.html', {'items': items, 'categories': categories})

    def seller_get(self, request):
        items = Item.get_all_items()
        categories = Category.get_all_categories()
        return render(request, 'dashboard/seller.html', {'items':items, 'categories':categories})
